...
import { CodeBlock } from "../messageRender.tsx";
        ...
        {!sel ? <div className="hint">{t("members.workspaceHint")}</div>
          : sel.error ? <div className="empty">{sel.error}</div>
            : <>
                ...
                {isMd && mode === "preview"
                  ? <div className="ws-md"><ReactMarkdown remarkPlugins={[remarkGfm, remarkBreaks]} rehypePlugins={[rehypeSanitize]} components={{ pre: ({ children }) => <CodeBlock>{children}</CodeBlock> }}>{sel.content || ""}</ReactMarkdown></div>
