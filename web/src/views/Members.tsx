...
import { CodeBlock, ColorSwatch, colorValueFromTag, GithubAlertBlockquote, markdownSchema, remarkColorSwatches, remarkGithubAlerts } from "../messageRender.tsx";
        ...
        {!sel ? <div className="hint">{t("members.workspaceHint")}</div>
          : sel.error ? <div className="empty">{sel.error}</div>
            : <>
                ...
                {isMd && mode === "preview"
                  ? <div className="ws-md"><ReactMarkdown remarkPlugins={[remarkGfm, remarkBreaks, remarkGithubAlerts, remarkColorSwatches]} rehypePlugins={[[rehypeSanitize, markdownSchema]]} components={{ a: ({ href, children }) => { const color = colorValueFromTag(href); return color ? <ColorSwatch value={color} /> : <a href={href} target="_blank" rel="noreferrer">{children}</a>; }, blockquote: ({ children, ...props }) => <GithubAlertBlockquote {...props}>{children}</GithubAlertBlockquote>, pre: ({ children }) => <CodeBlock>{children}</CodeBlock> }}>{sel.content || ""}</ReactMarkdown></div>
