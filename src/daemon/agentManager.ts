...
import { buildSystemPrompt, STARTUP_NUDGE, RESUME_NUDGE, DELIVERY_NUDGE, inboxNotice } from "./prompt.js";
...
const HERMES_DELIVER_DEBOUNCE_MS = Number(process.env.OPEN_TAG_HERMES_DELIVER_DEBOUNCE_MS ?? 500); // Hermes is one-shot; keep the fixed wait short when there is only one live notice.
...
interface AgentManagerOptions {
  ...
  hermesDeliverDebounceMs?: number;
...
export class AgentManager {
  ...
  private hermesDeliverDebounceMs: number;
  ...
  constructor(private send: (msg: unknown) => void, opts: AgentManagerOptions = {}) {
    ...
    this.hermesDeliverDebounceMs = opts.hermesDeliverDebounceMs ?? HERMES_DELIVER_DEBOUNCE_MS;
  ...
  private async startNow(agentId: string, config: AgentConfig): Promise<void> {
    ...
    // Deliveries that arrived earlier during workspace preparation are flushed just below, except Hermes
    // where the wakeup prompt itself is the concrete "check then send" turn.
    const pendingDeliveryCount = this.pendingDelivers.get(agentId)?.items.length ?? 0;
    const useHermesDeliveryNudge = runtime.name === "hermes" && pendingDeliveryCount > 0;
    ...
    running.session = runtime.start({
      ...
      initialPrompt: useHermesDeliveryNudge ? DELIVERY_NUDGE : (config.sessionId ? RESUME_NUDGE : STARTUP_NUDGE),
    }, cb);
    ...
    if (useHermesDeliveryNudge) {
      this.clearPendingDeliver(agentId);
      this.log.debug("pending deliver consumed by hermes wake nudge", { agentId, count: pendingDeliveryCount });
    } else {
  ...
  private debounceMsFor(r: Running): number {
    return (r.config.runtime ?? "").toLowerCase() === "hermes" ? this.hermesDeliverDebounceMs : this.deliverDebounceMs;
  }
